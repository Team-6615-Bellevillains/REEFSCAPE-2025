...
public class AprilTagDataUtil {
    ...
    private static final List<AprilTag> redAprilTags = aprilTagFieldLayout.getTags()
                                                            .stream()
                                                            .filter((aprilTag) -> getSide(aprilTag.pose.toPose2d()) == DriverStation.Alliance.Red)
                                                            .toList();
    private static final List<AprilTag> blueAprilTags = aprilTagFieldLayout.getTags()
                                                            .stream()
                                                            .filter((aprilTag) -> getSide(aprilTag.pose.toPose2d()) == DriverStation.Alliance.Blue)
                                                            .toList();

    private static final List<Pose2d> redCoralPoses = redAprilTags
                                            .stream()
                                            .filter((aprilTag) -> 6 <= aprilTag.ID && aprilTag.ID <= 11)
                                            .map((aprilTag) -> aprilTag.pose.toPose2d())
                                            .toList();
    private static final List<Pose2d> blueCoralPoses = blueAprilTags
                                            .stream()
                                            .filter((aprilTag) -> 17 <= aprilTag.ID && aprilTag.ID <= 22)
                                            .map((aprilTag) -> aprilTag.pose.toPose2d())
                                            .toList();

    public static List<Pose2d> getCoralAprilTagPoses(DriverStation.Alliance alliance) {
        switch (alliance) {
            case Red:
    ...
    public static Translation2d getReefCenter(DriverStation.Alliance alliance) {
        int tagIDA, tagIDB;
    ...
    private static DriverStation.Alliance getSide(Pose2d pose) {
